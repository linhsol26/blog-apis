...
const add = (req, res, next) => {
    ...
    req.body.article.slug =
        title.toLowerCase().split(" ").join("-") + `-${Date.now()}`;
...
const getOne = (req, res, next) => {
    ...
    ArticleModel.findOne({ slug })
        ...
        .then((document) => {
            if (document === null)
                return Promise.reject(new ApiError("Article not found"));
        ...
        .then((document) => {
            ...
            return req.user ?
                FollowingModel.findOne({
                    ...
                    followings: articleDocument.author,
                }) :
                null;
        })
        .then((document) => {
...
const getMany = async(req, res, next) => {
    ...
    if (author) {
        try {
            ...
            if (userDocument === null)
                return next(new ApiError(`User with username ${author} not found`));
    ...
    if (favorited && req.user) {
        try {
            const favoritedArticles = await FavoriteModel.find({
                users: req.user.userId,
            });
    ...
    ArticleModel.find(filterObject, null, {
            limit,
            skip: offset,
            sort: { createdAt: -1 },
        })
        .populate("author", { username: 1, bio: 1, image: 1, _id: 1 })
        .then((documents) => {
            ...
            return req.user ?
                FollowingModel.findOne({ user: req.user.userId }) :
                null;
        })
        .then((document) => {
            ...
            if (document === null) {
                ...
                articleDocuments.forEach((item) => {
                    item.author._doc.following = document.followings.includes(
                        item.author._id
                    );
                    delete item.author._id;
                });
